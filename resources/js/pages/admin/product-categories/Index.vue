...
const submit = () => {
    if (editingId.value) {
        form.put(`/admin/product-categories/${editingId.value}`, {
            onSuccess: () => {
                editingId.value = null;
                form.reset();
            },
...
<template>
    ...
    <div class="space-y-6 p-4 lg:p-8">
        <div>
            ...
            <h2 class="mt-1 font-serif text-3xl font-medium tracking-tight">Categorías de productos</h2>
        ...
        <form @submit.prevent="submit" class="card-elegant p-4">
            <div class="flex flex-wrap items-end gap-3">
                <div class="min-w-[160px] flex-1">
                    ...
                    <input v-model="form.name" required placeholder="Ej. Shampoos" class="input-elegant" />
                </div>
                <div class="w-20">
        ...
        <div v-if="categories.length" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            >
                ...
                <div class="mt-3">
                    ...
                    <p class="mt-1 text-xs text-mercury">{{ cat.products_count }} productos</p>
                </div>
        ...
        <div v-else class="card-elegant px-6 py-16 text-center">
            <Package class="mx-auto h-12 w-12 text-mercury/30" />
            <p class="mt-3 text-sm text-mercury">No hay categorías de productos registradas</p>
        </div>
    </div>
</template>
